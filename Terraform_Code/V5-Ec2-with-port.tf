...
resource "aws_instance" "demo-server" {
    ...
    key_name = "devop"
    vpc_security_group_ids = [aws_security_group.demo-sg.id]  # Correct security group reference
    subnet_id = aws_subnet.dpp-public-subnet-01.id  # Ensure instance is launched in the subnet
    for_each = toset(["jenkins-master", "jenkins-slave", "ansible"])     # Using the for_each meta-argument to create multiple resources dynamically

    tags = { # Defining tags for each resource created
    # Assigning a unique name to each resource based on the value of each.key
    Name = "${each.key}"
}
...
resource "aws_security_group" "demo-sg" {
    ...
    ingress {
     ...
     ingress {
        description = "jenkins port"  # Description of the rule
        from_port = 8080  # Allow incoming traffic on port 8080 (SSH)
        to_port = 8080
        protocol = "tcp"  # TCP protocol
        cidr_blocks = ["0.0.0.0/0"]  # Open to all IP addresses (not recommended for production)
    }
